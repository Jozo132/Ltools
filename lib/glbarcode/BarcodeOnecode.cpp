...
namespace glbarcode
{
	...
	std::string BarcodeOnecode::encode( const std::string& cookedData )
	{
		...
		for ( int i = 0; i < 10; i++ )
		{
			character[i] = BC_OC::characterTable[ codeword[i] ];
		...
		for ( int i = 0; i < 65; i++ )
		{
			int d = (character[ BC_OC::barMap[i].descender.i ] & BC_OC::barMap[i].descender.mask) != 0 ? 1 : 0;
			int a = (character[ BC_OC::barMap[i].ascender.i ]  & BC_OC::barMap[i].ascender.mask)  != 0 ? 1 : 0;

			code += BC_OC::tdafTable[ (a<<1) + d ];
		}
	...
	void BarcodeOnecode::vectorize( const std::string& codedData,
					...
					double&            h )
	{
		double x = BC_OC::ONECODE_HORIZ_MARGIN;
		for ( unsigned int i = 0; i < codedData.size(); i++ )
		{
			double y = BC_OC::ONECODE_VERT_MARGIN;
			double length;

			switch ( codedData[i] )
			{
			case 'T':
				y      += BC_OC::ONECODE_TRACKER_OFFSET;
				length  = BC_OC::ONECODE_TRACKER_HEIGHT;
				break;
			case 'D':
				y      += BC_OC::ONECODE_DESCENDER_OFFSET;
				length  = BC_OC::ONECODE_DESCENDER_HEIGHT;
				break;
			case 'A':
				y      += BC_OC::ONECODE_ASCENDER_OFFSET;
				length  = BC_OC::ONECODE_ASCENDER_HEIGHT;
				break;
			case 'F':
				y      += BC_OC::ONECODE_FULL_OFFSET;
				length  = BC_OC::ONECODE_FULL_HEIGHT;
				break;
			default:
				// Not reached
				break;
			}
			double width = BC_OC::ONECODE_BAR_WIDTH;

			addLine( x, y, width, length );

			x += BC_OC::ONECODE_BAR_PITCH;
		}

		/* Overwrite requested size with actual size. */
		w = x + BC_OC::ONECODE_HORIZ_MARGIN;
		h = BC_OC::ONECODE_FULL_HEIGHT + 2 * BC_OC::ONECODE_VERT_MARGIN;
	}
