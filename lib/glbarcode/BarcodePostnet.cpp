...
namespace glbarcode {
	...
	std::string BarcodePostnet::encode(const std::string& cookedData)
	{
		...
		for (unsigned int i = 0; i < cookedData.size(); i++)
		{
			if (isdigit(cookedData[i]))
			{
				...
				code += BC_PN::symbols[d];
				sum += d;
			}
		}

		/* Create mandatory correction character */
		code += BC_PN::symbols[(10 - (sum % 10)) % 10];
	...
	void BarcodePostnet::vectorize(const std::string& codedData,
		...
		double& h)
	{
		double x = BC_PN::POSTNET_HORIZ_MARGIN;
		for (unsigned int i = 0; i < codedData.size(); i++)
		{
			double length, width;

			double y = BC_PN::POSTNET_VERT_MARGIN;

			switch (codedData[i])
			{
				case '0':
					y += BC_PN::POSTNET_FULLBAR_HEIGHT - BC_PN::POSTNET_HALFBAR_HEIGHT;
					length = BC_PN::POSTNET_HALFBAR_HEIGHT;
					break;
				case '1':
					length = BC_PN::POSTNET_FULLBAR_HEIGHT;
					break;
				default:
					// Not reached
					break;
			}
			width = BC_PN::POSTNET_BAR_WIDTH;

			addLine(x, y, width, length);

			x += BC_PN::POSTNET_BAR_PITCH;
		}

		/* Overwrite requested size with actual size. */
		w = x + BC_PN::POSTNET_HORIZ_MARGIN;
		h = BC_PN::POSTNET_FULLBAR_HEIGHT + 2 * BC_PN::POSTNET_VERT_MARGIN;
	}
